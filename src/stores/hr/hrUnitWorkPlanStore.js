// src/stores/hrUnitWorkPlanStore.js
import { defineStore } from 'pinia'
import { api } from 'boot/axios'
import { ref, computed } from 'vue'

export const useHRUnitWorkPlanStore = defineStore('hrUnitWorkPlan', () => {
  const offices = ref([])
  const divisions = ref([])
  const employees = ref([])
...
const fetchDivisionsWithWorkPlans = async (officeId, targetPeriod = null) => {
  try {
    ...
    const response = await api.get('/hr/unit_work_plan/divisions', { params });
...
const fetchEmployeesByDivision = async (division, officeId, targetPeriod) => {
  try {
    ...
    const response = await api.get('/hr/unit_work_plan/employees', {
      params: {
        division,
        office_id: officeId,
  ...
  const fetchUnitWorkPlans = async (division, targetPeriod) => {
    try {
      loading.value = true
      const response = await api.get('/hr/unit_work_plan', {
