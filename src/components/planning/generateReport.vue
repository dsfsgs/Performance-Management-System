    ...
    <div class="main-content">
      ...
      <div class="division-nav">
        ...
        <q-list padding class="division-list">
          <q-item v-for="(division, index) in filteredDivisions || store.divisions" :key="index" clickable v-ripple
            :active="store.selectedDivision === division.division"
            @click="selectDivision(division.division, division.target_period || division.full_period)"
            active-class="active-division">
            <q-item-section>
              <q-item-label>{{ division.division }}</q-item-label>
              <q-item-label caption>{{ division.target_period }} {{ division.year }}</q-item-label>
            </q-item-section>
          </q-item>
        </q-list>
...
import { useplanning_unit_work_planStore } from 'src/stores/planning/planning_unit_work_planStore'
...
export default {
  ...
  setup(props) {
    const store = useplanning_unit_work_planStore();
    ...
    watch(() => props.officeId, async (newOfficeId) => {
      if (newOfficeId && props.targetPeriod) {
        await loadInitialData();
      }
    });

    watch(() => props.targetPeriod, async (newPeriod) => {
      if (props.officeId && newPeriod) {
        await loadInitialData();
      }
    });
    ...
    const loadInitialData = async () => {
      try {
        ...
        await store.fetchDivisionsWithWorkPlans(props.officeId, props.targetPeriod);

        // If filteredDivisions prop is provided, use it
        const divisions = props.filteredDivisions || store.divisions;

        if (divisions && divisions.length > 0) {
          // Select the first division by default
          await selectDivision(
            divisions[0].division,
            divisions[0].target_period || divisions[0].full_period
          );
        }
      } catch (error) {
    ...
    const selectFirstDivisionAvailable = async () => {
      ...
      if (divisions && divisions.length > 0) {
        ...
        await selectDivision(
          ...
          firstDivision.target_period || firstDivision.full_period
        )
      }
    }
    onMounted(async () => {
      if (props.officeId && props.targetPeriod) {
        await loadInitialData();
      }
    });
    onMounted(async () => {
      try {
        // Make sure we have both officeId and targetPeriod before fetching
        if (props.officeId && props.targetPeriod) {
          // Fetch divisions with work plans for the selected office and target period
          await store.fetchDivisionsWithWorkPlans(props.officeId, props.targetPeriod)
    ...
    // Watch for changes in the divisions array
    watch(() => props.filteredDivisions || store.divisions, async (newDivisions) => {
      if (newDivisions && newDivisions.length > 0 && !store.selectedDivision) {
        await selectFirstDivisionAvailable()
      }
    }, { deep: true })

    // Competency code mappings
    ...
    // Format competency data with code mappings
    const formatCompetency = (competency, type) => {
      if (!competency) return 'N/A';
      try {
        const parsed = typeof competency === 'string' ? JSON.parse(competency) : competency;
        const mappings = competencyMappings[type] || {};

        return Object.entries(parsed)
          .map(([key, value]) => {
            const code = mappings[key] || key;
            return `${code}: ${value}`;
          })
          .join('<br>');
      } catch {
    ...
    // Format standards with Q-, T-, E- prefixes
    const formatStandards = (standards, level) => {
      if (!standards) return 'N/A';
      try {
        // Parse the standards (handle both string and already parsed)
        const parsed = typeof standards === 'string' ? JSON.parse(standards) : standards;

        // Find the standard for the requested level
        const standard = Array.isArray(parsed)
          ? parsed.find(item => item.rating === level.toString())
          : parsed[`level_${level}`];

        if (!standard) return 'N/A';

        // Format the standard values with Q-, T-, E- prefixes
        return [
          standard.quantity ? `Q-${standard.quantity}` : '',
          standard.timeliness ? `T-${standard.timeliness}` : '',
          standard.effectiveness ? `E-${standard.effectiveness}` : ''
        ].filter(Boolean).join('<br>') || 'N/A';
      } catch (e) {
        console.error('Error formatting standards:', e);
        return 'N/A';
      }
    };
    const selectDivision = async (division, targetPeriod) => {
      try {
        let formattedPeriod = targetPeriod;

        store.setSelectedDivision(division);
        store.setSelectedTargetPeriod(formattedPeriod);

        const divisionData = await store.fetchDivisionDetails(
          division,
          props.officeId,
          formattedPeriod
        );
        ...
        currentDivisionData.value = {
          ...
          employees: divisionData.employees.map(employee => ({
            ...employee,
            outputs: employee.unit_work_plans.map(plan => ({
              // Use MFO if available, otherwise use Output
              name: plan.mfo || plan.output || 'N/A',
              core: formatCompetency(plan.core, 'core'),
              technical: formatCompetency(plan.technical, 'technical'),
              leadership: formatCompetency(plan.leadership, 'leadership'),
              indicator: plan.success_indicator || 'N/A',
              required: plan.required_output || 'N/A',
              standard5: formatStandards(plan.standard_outcomes, 5),
              standard4: formatStandards(plan.standard_outcomes, 4),
              standard3: formatStandards(plan.standard_outcomes, 3),
              standard2: formatStandards(plan.standard_outcomes, 2),
              standard1: formatStandards(plan.standard_outcomes, 1)
            }))
          }))
        };
      } catch (error) {
        ...
        if (typeof store.$q !== 'undefined') {
          store.$q.notify({
            type: 'negative',
            message: error.message || 'Failed to load division data. Please try again.'
          });
        }
      }
    };
